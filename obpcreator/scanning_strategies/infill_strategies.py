...
def line_snake(point_infill, scan_settings):
    coord_matrix = point_infill.coord_matrix
    keep_matrix = point_infill.keep_matrix
    ...
    for i in range(len(snake_sorted_left)):
        ...
        if start_coord==end_coord:
            ...
            obp_elements.append(obp.TimedPoints([a], [scan_settings.scan_parameters.dwell_time], bp))
        else:
            a = obp.Point(start_coord.real*1000, start_coord.imag*1000)
...
def line_left_right(point_infill, scan_settings):
    coord_matrix = point_infill.coord_matrix
    keep_matrix = point_infill.keep_matrix
    left, right = find_start_end_line_positions(keep_matrix)
    obp_elements = []
...
def line_right_left(point_infill, scan_settings):
    coord_matrix = point_infill.coord_matrix
    keep_matrix = point_infill.keep_matrix
    left, right = find_start_end_line_positions(keep_matrix)
...
def point_random(point_infill, scan_settings):
    coord_matrix = point_infill.coord_matrix
    keep_matrix = point_infill.keep_matrix
